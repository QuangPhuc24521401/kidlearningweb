...
const LESSONS_LOCAL = {
  ...
  /* ─── ÂM NHẠC (dùng dạng câu hỏi có sẵn: classify/sequence/count) ─── */
  am_nhac:[
    {type:'classify',q:'Nhạc cụ nào dùng để gõ?', v:'Nhạc cụ nào dùng để gõ', ans:['🥁 Trống','🎻 Vĩ cầm','🎺 Kèn'], cor:'🥁 Trống'},
    {type:'classify',q:'Nhạc cụ nào dùng để thổi?', v:'Nhạc cụ nào dùng để thổi', ans:['🎺 Kèn','🥁 Trống','🎸 Đàn'], cor:'🎺 Kèn'},
    {type:'classify',q:'Đâu là nốt nhạc?', v:'Đâu là nốt nhạc', ans:['🎵 Nốt nhạc','🐶 Chó','🍎 Táo'], cor:'🎵 Nốt nhạc'},
    {type:'sequence',seq:[1,2,3,null,5], q:'Đếm nhịp: 1, 2, 3, ?, 5', v:'Đếm nhịp, số nào còn thiếu', ans:['4','6','2'], cor:'4'},
    {type:'sequence',seq:[2,4,6,null,10], q:'Nhịp đôi: 2, 4, 6, ?, 10', v:'Số nào còn thiếu', ans:['7','8','9'], cor:'8'},
    {type:'count',item:'star',count:3, q:'Có bao nhiêu nốt nhạc? (đếm hình ⭐ thay cho nốt)', v:'Con hãy đếm số hình', ans:['2','3','4'], cor:'3'},
  ],

  /* ─── GHÉP HÌNH / KHÔNG GIAN (oddone/sort/sequence) ─── */
  ghep_hinh:[
    {type:'oddone',q:'Mảnh nào khác hình?', v:'Mảnh nào khác hình', shapes:['square','square','triangle','square'], colors:['#6c8eff','#6c8eff','#6c8eff','#6c8eff'], cor:'2'},
    {type:'oddone',q:'Mảnh nào khác màu?', v:'Mảnh nào khác màu', shapes:['circle','circle','circle','circle'], colors:['#ff6b6b','#ff6b6b','#7ee8a2','#ff6b6b'], cor:'2'},
    {type:'sort',q:'Mảnh nào nhỏ nhất?', v:'Mảnh nào nhỏ nhất', sizes:[78,42,92], color:'#ff6b6b', shape:'square', ans:['Hình 1','Hình 2','Hình 3'], cor:'Hình 2'},
    {type:'sort',q:'Mảnh nào lớn nhất?', v:'Mảnh nào lớn nhất', sizes:[55,96,40], color:'#6c8eff', shape:'triangle', ans:['Hình 1','Hình 2','Hình 3'], cor:'Hình 2'},
    {type:'sequence',seq:[1,3,null,7,9], q:'Số nào còn thiếu? 1, 3, ?, 7, 9', v:'Số nào còn thiếu', ans:['4','5','6'], cor:'5'},
  ],

  /* ─── MỸ THUẬT (color/classify/sequence) ─── */
  my_thuat:[
    {type:'color',color:'#ff6b6b',name:'đỏ', q:'Đây là màu gì?', v:'Đây là màu gì', ans:['Màu đỏ','Màu xanh','Màu vàng'], cor:'Màu đỏ'},
    {type:'color',color:'#7ee8a2',name:'xanh lá', q:'Đây là màu gì?', v:'Đây là màu gì', ans:['Màu tím','Màu xanh lá','Màu cam'], cor:'Màu xanh lá'},
    {type:'classify',q:'Màu nào giống bầu trời?', v:'Màu nào giống bầu trời', ans:['🔵 Xanh dương','🔴 Đỏ','🟡 Vàng'], cor:'🔵 Xanh dương'},
    {type:'sequence',seq:[1,2,null,2,1], q:'Hoa văn đối xứng: 1, 2, ?, 2, 1', v:'Số nào còn thiếu', ans:['1','2','3'], cor:'3'},
  ],

  /* ─── NGÔN NGỮ (classify) ─── */
  ngon_ngu:[
    {type:'classify',q:'Con nào là động vật?', v:'Con nào là động vật', ans:['🐱 Mèo','📗 Sách','🍎 Táo'], cor:'🐱 Mèo'},
    {type:'classify',q:'Đâu là trái cây?', v:'Đâu là trái cây', ans:['🍌 Chuối','🚗 Xe hơi','🏠 Ngôi nhà'], cor:'🍌 Chuối'},
    {type:'classify',q:'Đâu là đồ dùng học tập?', v:'Đâu là đồ dùng học tập', ans:['✏️ Bút chì','🍕 Pizza','⚽ Bóng'], cor:'✏️ Bút chì'},
    {type:'classify',q:'Đâu là phương tiện?', v:'Đâu là phương tiện', ans:['🚲 Xe đạp','🐟 Cá','🌸 Hoa'], cor:'🚲 Xe đạp'},
  ]
};
...
/* ─── HOVER TTS (đọc đáp án khi di chuột / focus) ─── */
let answerHoverTimer = null;
const ORD_VI = ['một','hai','ba','bốn','năm','sáu','bảy','tám','chín','mười'];
function ordinalVi(i){ return ORD_VI[i] || String(i + 1); }
function shapeNameVi(sh){
  const m = { circle:'Hình tròn', square:'Hình vuông', triangle:'Tam giác', rectangle:'Hình chữ nhật', diamond:'Hình thoi' };
  return m[sh] || 'Hình';
}
function scheduleSpeakAnswer(text){
  clearTimeout(answerHoverTimer);
  const t = String(text || '').trim();
  if(!t) return;
  answerHoverTimer = setTimeout(()=>{ answerHoverTimer = null; speak(t); }, 320);
}
function cancelScheduleSpeakAnswer(){ clearTimeout(answerHoverTimer); answerHoverTimer = null; }
function bindAnswerHover(el, ttsText){
  const run = ()=> scheduleSpeakAnswer(ttsText);
  el.addEventListener('pointerenter', run);
  el.addEventListener('pointerleave', cancelScheduleSpeakAnswer);
  el.setAttribute('tabindex','0');
  el.addEventListener('focus', run);
  el.addEventListener('blur', cancelScheduleSpeakAnswer);
}
...
function showLesson(){
  ...
  if(lesson.type==='oddone'){
    lesson.shapes.forEach((sh,i)=>{
      ...
      bindAnswerHover(div, `${shapeNameVi(sh)}, ô thứ ${ordinalVi(i)}`);
