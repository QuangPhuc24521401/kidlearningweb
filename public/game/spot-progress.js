...
(function (global) {
  ...
  var SUBJECT_KEY = 'game_spot';

  function topicKey(levelId) { return 'Tìm khác biệt ' + levelId; }

  function levelsList() {
    return (global.SpotLevels && global.SpotLevels.LEVELS) || [];
  }
  ...
  function saveLevelResult(level, opts) {
    ...
    try {
      if (opts.finished && typeof global.syncAchievementsAfterGameSave === 'function') {
        global.syncAchievementsAfterGameSave('spot');
      }
    } catch (e) {}
    return entry;
  }
