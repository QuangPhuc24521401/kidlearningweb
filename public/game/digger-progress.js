/* ═══════════════════════════════════════════════════
   DIGGER-PROGRESS.JS — Tiến độ game đào vàng
═══════════════════════════════════════════════════ */
(function (global) {
  'use strict';

  var SUBJECT_KEY = 'game_digger';

  function topicKey(levelId) { return 'Đào vàng ' + levelId; }

  function levelsList() {
    return (global.DiggerLevels && global.DiggerLevels.LEVELS) || [];
  }
  ...
  function saveLevelResult(level, opts) {
        ...
        if (user && firebase.firestore) {
          if (typeof KidProgressSync !== 'undefined' && KidProgressSync.pushToCloud) {
            ...
            firebase.firestore().collection('learning_progress').doc(user.uid)
              ...
              .catch(function (err) { console.warn('[digger-progress]', err); });
          }
        }
      }
    } catch (e) {}
    ...
    try {
      if (opts.finished && typeof global.syncAchievementsAfterGameSave === 'function') {
        global.syncAchievementsAfterGameSave('digger');
      }
    } catch (e) {}
