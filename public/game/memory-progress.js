...
(function (global) {
  ...
  var SUBJECT_KEY = 'game_memory';

  function topicKey(levelId) { return 'Ghép cặp ' + levelId; }

  function levelsList() {
    return (global.MemoryLevels && global.MemoryLevels.LEVELS) || [];
  }
  ...
  function saveLevelResult(level, opts) {
    ...
    try {
      if (opts.finished && typeof global.syncAchievementsAfterGameSave === 'function') {
        global.syncAchievementsAfterGameSave('memory');
      }
    } catch (e) {}
    return entry;
  }

  global.MemoryProgress = {
