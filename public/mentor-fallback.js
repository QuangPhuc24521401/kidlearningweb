/* Trả lời mẫu tiếng Việt — dùng khi Gemini chậm/lỗi (client + logic giống server) */
(function (global) {
  'use strict';
  ...
  function reply(message) {
    ...
    if (t.includes('mat troi') || raw.includes('mặt trời')) {
      return 'Mặt trời là <strong>hình tròn</strong> nha con! ☀️ Tròn tròn như quả bóng lớn trên trời, màu vàng ấm. Con thử vẽ một vòng tròn vàng xem!';
    }
    if (t.includes('mat trang') || raw.includes('mặt trăng')) {
      return 'Mặt trăng cũng là <strong>hình tròn</strong> nha con! 🌙 Đêm nào con có nhìn trăng tròn trên trời không?';
    }
    ...
    if ((t.includes('hinh gi') || t.includes('hinh dang')) && (t.includes('mat troi') || t.includes('bong') || t.includes('dong xu'))) {
      return 'Đó là <strong>hình tròn</strong> nha con! ⭕ Tròn tròn, lăn được, không có góc nhọn.';
    }
    ...
    if (t.includes('qua bong') || raw.includes('quả bóng') || t.includes('bong da')) {
      return 'Quả bóng là <strong>hình tròn</strong> nha con! ⚽ Tròn tròn, lăn được khắp nơi!';
    }

    return 'Cô nghe câu hỏi của con rồi! 😊 Con thử hỏi về <strong>hình tròn</strong>, <strong>màu sắc</strong> hoặc <strong>đếm số</strong> — cô giải thích dễ hiểu lắm!';
  }

  global.KidMentorFallback = { reply: reply, norm: norm };
})(window);
